...
- Usage accounting must use explicit snapshots and one canonical merge strategy, quarantine any blank/unknown attribution rows regardless of usage value, and block SPC/delta publication when unknown/blank attribution exceeds threshold or stratum baselines collapse to zero-variance limits. Additionally, fail closed when stratified aggregates (model/provider) do not reconcile with per-iteration current-run totals
...
- Dirty-worktree precondition failures are environment failures: fail fast before bead attempt, do not consume bead retry budget, and require remediation before requeue
...
- On bead failure: add to `skippedBeads`. For broad/high-risk scope (cross-package, umbrella titles, or >=6 touched files), decompose after the first failure; otherwise decompose after 2 consecutive failures. If failure signature is invocation/pre-launch OR (duration_ms=0 AND input_tokens=0) OR gate_block_reason is `dirty_worktree_precondition`, forbid same-scope retry and require immediate diagnostic remediation/decomposition. Environment failures (`dirty_worktree_precondition`) must not consume bead retry budget. Create/link decomposition sub-beads with expected_outputs and bounded scope. Block parent retries until at least one child lands and is linked; no retries on partial/non-idempotent decomposition; emit decomposition-attempt event; fail if skipped; skip escalation after 3+.
...
- Before build phase starts, run a hard preflight environment gate (clean worktree, queue-state writeability, spec-branch checkout viability). If preflight fails, terminate run as `environment_blocked` before bead attempt. Build phases then run `go test`/`go vet` on touched packages only. Trigger full validation when either (a) every N successes OR (b) risk signal fires (cross-package touch, architecture/runner/provider paths, control-limit build-failure signal, or any validation command timeout). Repeated identical timeout signatures for the same bead must force immediate decomposition/fallback and block same-scope retry
