...
func TestCodexStreamSuccessFixtureHasOrderedLifecycle(t *testing.T) {
	// Expected failure: codex_stream_success.jsonl currently has only 2 JSON events
	// (assistant + result) and lacks a start event. The implementation must add a
	// start event (e.g., "response.started") to form a 3-event lifecycle.
...
func TestCodexStreamFailureFixtureTerminatesWithError(t *testing.T) {
	// Expected failure: codex_stream_failure.jsonl currently ends with a "result"
	// event (type="result") rather than an explicit error event type. The
	// implementation must restructure to use a terminal error event (e.g.,
	// "error", "stream_error", or "response.error") with a non-empty error field.
...
func TestCodexStreamFixturesHaveProvenanceAndRefreshComments(t *testing.T) {
	// Expected failure: both codex JSONL stream fixtures currently have only
	// a single "# provenance:" comment. The implementation must add a second
	// "# refresh:" comment line to each fixture.
	fixtures := []string{
