...
func TestDecomposeCohortSelector_ValidateOverrides_PlanRequired(t *testing.T) {
	ctx := context.Background()
	plansDir := t.TempDir()
	required := []string{"spec-alpha", "spec-bravo", "spec-charlie", "spec-delta", "spec-await"}
	for _, spec := range required {
		createPlanFile(t, plansDir, spec)
	}
	// omit spec-missing plan file
	creates := map[string][]*bead.Bead{
		"spec:spec-alpha":   makeBeads("closed"),
		"spec:spec-bravo":   makeBeads("closed"),
		"spec:spec-charlie": makeBeads("closed"),
		"spec:spec-delta":   makeBeads("closed"),
		"spec:spec-await":   makeBeads("closed"),
		"spec:spec-missing": makeBeads("closed"),
	}
	store := &fakeLabelLister{
		beads: creates,
	}
	...
	selector := NewDecomposeCohortSelector(store, plansDir)
	overrides := []string{"spec-alpha", "spec-missing", "spec-bravo", "spec-charlie", "spec-delta"}

	err := selector.ValidateOverrides(ctx, overrides)
	if err == nil {
		t.Fatal("expected error for missing plan file")
	}
	if !stdstrings.Contains(err.Error(), "plan file") {
		t.Fatalf("unexpected error: %v", err)
	}
}
