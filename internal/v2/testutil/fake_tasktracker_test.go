...
func TestFakeTaskTracker_CreatesBeadsAndObservesDependencies(t *testing.T) {
    ...
    first, err := tracker.CreateBead(ctx, "first", "desc", 1, []string{"alpha"}, nil)
    if err != nil {
    ...
    second, err := tracker.CreateBead(ctx, "second", "desc", 2, []string{"beta"}, []string{first.ID})
    if err != nil {
        t.Fatalf("CreateBead failed: %v", err)
    }

    refreshed, err := tracker.ShowBead(ctx, first.ID)
    if err != nil {
    ...
    queried, err := tracker.QueryBeads(ctx, []string{"alpha"}, tracker.StatusOpen, "")
    if err != nil {
    ...
    if pending, err := tracker.NextBead(ctx); err != nil {
    ...
    if err := tracker.CloseBead(ctx, first.ID); err != nil {
        t.Fatalf("CloseBead failed: %v", err)
    }

    unlocked, err := tracker.NextBead(ctx)
    if err != nil {
        t.Fatalf("NextBead failed: %v", err)
    }
    ...
    shownSecond, err := tracker.ShowBead(ctx, second.ID)
    if err != nil {
        t.Fatalf("ShowBead failed: %v", err)
    }
