...
func (f *fakeRemediationRunner) Run(_ context.Context, _, _ string, _ []stagepkg.SpecFinding) error {
	f.calls++
	return f.err
}
...
func (r *recordingRemediationRunner) Run(_ context.Context, specID, worktree string, findings []stagepkg.SpecFinding) error {
	r.calls++
	r.lastSpecID = specID
...
func newFakeSpecReviewStage(results ...stagepkg.Result) *fakeSpecReviewStage {
	copied := append([]stagepkg.Result(nil), results...)
	if len(copied) == 0 {
		copied = append(copied, stagepkg.Result{Decision: stagepkg.DecisionProceed})
	}
	for i := range copied {
		if copied[i].Decision == 0 {
			copied[i].Decision = stagepkg.DecisionProceed
		}
	}
