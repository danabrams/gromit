...
// TestProcessCodexStreamMapsMessageOutputTextDeltaToEventContent verifies that
// message.output_text.delta events map to StreamEvent type "EventContent" with the delta text.
func TestProcessCodexStreamMapsMessageOutputTextDeltaToEventContent(t *testing.T) {
	t.Parallel()
	input := `{"type":"message.output_text.delta","text":"delta text"}` + "\n"
	reader := strings.NewReader(input)
	var output bytes.Buffer
	var receivedEvents [][]byte
	...
	if len(receivedEvents) == 0 {
		t.Fatal("EventHandler was not called for message.output_text.delta event")
	}
	...
	if parsed.Type != "EventContent" {
		t.Errorf("message.output_text.delta mapped to type %q, want %q", parsed.Type, "EventContent")
	}
	if parsed.Text != "delta text" {
		t.Errorf("text = %q, want %q", parsed.Text, "delta text")
	}
}
...
// TestProcessCodexStreamMapsToolCallCreatedToEventToolUse verifies that tool_call.created
// events map to StreamEvent type "EventToolUse" with tool call payload.
func TestProcessCodexStreamMapsToolCallCreatedToEventToolUse(t *testing.T) {
	t.Parallel()
	input := `{"type":"tool_call.created","tool_call":{"tool_name":"search","input":{"query":"foo"}}}` + "\n"
	reader := strings.NewReader(input)
	var output bytes.Buffer
	var receivedEvents [][]byte
	...
	if len(receivedEvents) == 0 {
		t.Fatal("EventHandler was not called for tool_call.created event")
	}
	...
	if parsed.Type != "EventToolUse" {
		t.Errorf("tool_call.created mapped to type %q, want %q", parsed.Type, "EventToolUse")
	}
