...
func buildFixPlanPrompt(req FixPlanRequest) string {
	...
	b.WriteString("- If a failure message starts with `[suspect-proof-check]`, do NOT create a code implementation task. Instead, create a proof-check rewrite task that replaces the failing pattern-matching check with a runtime behavioral check (e.g. `./binary subcommand --help | grep -q -- '--flag-name'`).\n")
