...
func buildFixPlanPrompt(req FixPlanRequest) string {
	...
	b.WriteString("- If a failure message starts with `[suspect-proof-check]`, do NOT create a code implementation task. Instead, describe a proof-check rewrite that swaps the failing pattern-matching check for a runtime behavioral check (e.g. `./binary subcommand --help | grep -q -- '--flag-name'`) and leave the implementation untouched.\n")
	...
	b.WriteString("    - **Rule 7 (Runtime over source-grep for behavioral properties)**: For CLI flags, subcommands, or user-visible behaviors, check the built binary: `./binary subcommand --help | grep -q -- '--flag-name'`. Source patterns vary by language/framework. Use source grep only for call sites and ordering where no runtime check is possible.\n")
	b.WriteString("    - **[suspect-proof-check]**: If a failure message starts with `[suspect-proof-check]`, do NOT create a code implementation task. Instead, create a proof-check rewrite task that replaces the failing pattern-matching check with a runtime behavioral check (e.g. `./binary subcommand --help | grep -q -- '--flag-name'`).\n")
...
func buildPlanPrompt(req PlanRequest) string {
	...
	b.WriteString("\n7. **Rule 7 (Runtime over source-grep for behavioral properties)**: When verifying that a CLI flag, subcommand, API endpoint, or other user-visible behavior exists, check the *running artifact*, not the source code. Source patterns vary by language and framework (`\"title\"` in Go/cobra, `@click.option('--title')` in Python, `.option('--title')` in JS/commander). The built binary is canonical. Example: `./binary subcommand --help | grep -q -- '--flag-name'`. Use source grep only for implementation structure (call sites, ordering) where no runtime check is possible.\n")
	b.WriteString("\n## Suspect Proof-Check Guidance\n")
	b.WriteString("If a failure message begins with `[suspect-proof-check]`, do NOT create an implementation task for that failure. Instead, describe a proof-check rewrite task that replaces the failing pattern-matching check with a runtime behavioral check (for example, `./binary subcommand --help | grep -q -- '--flag-name'`).\n")
	return b.String()
}
