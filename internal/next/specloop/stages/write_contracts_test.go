...
func TestWriteContracts_SpecificityRetryStructuralRegression(t *testing.T) {
	...
	// Invalid contract: has scenario but ContractAssertion has zero fields set (fails structural validation)
	invalidContract := &contract.ScenarioContract{
		Scenarios: []contract.ScenarioAssertions{
			{
				Name: "add-works",
	...
	// Verify writer was called exactly twice (1 initial + 1 specificity retry)
	if writerCalls != 2 {
		t.Fatalf("expected exactly 2 writer calls, got %d", writerCalls)
	}

	// Verify the retry spec packet contains specificity warning text
	if !strings.Contains(retrySpecPacket, "Specificity") {
		t.Fatalf("expected retry spec packet to contain 'Specificity', got: %s", retrySpecPacket)
	}
