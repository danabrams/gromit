...
// TestRunnerLabelFiltering_MultipleIterationsWithLabelFilters verifies that across
// multiple iterations, the runner consistently uses ReadyWithLabel and never falls back
// to Ready() when label filters are active
func TestRunnerLabelFiltering_MultipleIterationsWithLabelFilters(t *testing.T) {
	var output strings.Builder
	...
	allBeads := []*bead.Bead{
		...
		{ID: "pay-2", Title: "Payment task 2", Priority: 2, Labels: []string{"spec:payments"}, ExpectedOutputs: []string{}},
	...
	if readyCalled {
		t.Error("Ready() was called during execution, but it should never be called when label filters are active")
	}

	// Verify only auth and payment beads were processed (not "other-1")
	expectedBeads := []string{"auth-2", "auth-1", "pay-1", "pay-2"} // Priority order: P0, P1, P1, P2
	...
	if len(readyWithLabelCalls) < 4 {
		t.Errorf("Expected ReadyWithLabel to be called at least 4 times (across multiple iterations), got %d calls", len(readyWithLabelCalls))
	}
