...
func TestDecomposerAdapter_Decompose_CreatesChildBeads(t *testing.T) {
	...
	adapter := &decomposerAdapter{tracker: bead.NewBDAdapter(client), beads: client, router: stubRouter}
	b := &bead.Bead{
		ID:              "over-1",
...
func TestDecomposerAdapter_InvokesProviderViaRouter(t *testing.T) {
	...
	adapter := &decomposerAdapter{tracker: bead.NewBDAdapter(client), beads: client, router: router}
	b := &bead.Bead{
		ID:              "parent-1",
...
func TestGateRunScopeGate_ContractViolationFallsBackToBlock(t *testing.T) {
	...
	gate := prepare.New(io.Discard).WithDecomposer(&decomposerAdapter{tracker: bead.NewBDAdapter(client), beads: client, router: router})
	blockTrue := true
	cfg := &config.Config{
...
func TestNewRunnerImpl_AllowsReadyBeadWhenReadinessEnabled(t *testing.T) {
	t.Parallel()
	...
	orch.cfg.GetBead = func(ctx context.Context) (*bead.Bead, error) {
		...
		return &bead.Bead{
			ID:              "ready-bead",
			Title:           "Ready Bead",
			ExpectedOutputs: []string{"deliverable"},
		}, nil
	}
	...
	if !buildInvoked {
		t.Fatal("build stage did not run despite bead being ready")
	}
}
