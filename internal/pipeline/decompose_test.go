...
func TestDecomposeWorkflow_SkipValidationUnlimitedMaxAllowsLargeBatch(t *testing.T) {
	tmpDir := t.TempDir()
	plansDir := filepath.Join(tmpDir, "plans")
	if err := os.MkdirAll(plansDir, 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(plansDir, "skip-validation-unlimited.md"), []byte("# Skip Validation Unlimited"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if createCalls != 6 {
		t.Fatalf("bead create calls = %d, want 6", createCalls)
	}
}

func TestDecomposeWorkflow_SkipValidationUndersizedBatchErrorsWithoutCreatingBeads(t *testing.T) {
	tmpDir := t.TempDir()
	plansDir := filepath.Join(tmpDir, "plans")
	if err := os.MkdirAll(plansDir, 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(plansDir, "skip-validation-undersized.md"), []byte("# Skip Validation Undersized"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	if err == nil {
		t.Fatal("Decompose() returned nil error, want batch_size_min contract violation")
	}
	if !strings.Contains(err.Error(), "decomposition contract violation: batch_size_min") {
		t.Fatalf("error = %v, want explicit batch_size_min contract violation format", err)
	}
	if createCalls != 0 {
		t.Fatalf("bead create calls = %d, want 0", createCalls)
	}
...
func TestDecomposeWorkflow_RetriesWhenBroadScopeSignalFlagsHighComplexity(t *testing.T) {
	tmpDir := t.TempDir()
	plansDir := filepath.Join(tmpDir, "plans")
	if err := os.MkdirAll(plansDir, 0755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(plansDir, "complexity-broad-scope-retry.md"), []byte("# Complexity Broad Scope Retry"), 0644); err != nil {
		t.Fatal(err)
	}
	...
	mockClaude := &decomposeAcceptanceClaudeClient{
		runFunc: func(prompt string, model string) (*ClaudeRunResult, error) {
			...
			return &ClaudeRunResult{
				...
				ExitCode: 0,
				Output: `[
					{"title":"Split auth step 1","description":"narrow scope","priority":"P1","estimated_files":1,"acceptance_criteria":["a"],"depends_on_index":[]},
					{"title":"Small task","description":"d","priority":"P1","estimated_files":1,"acceptance_criteria":["b"],"depends_on_index":[]}
				]`,
			}, nil
		},
	...
	if !strings.Contains(secondPrompt, "broad-scope language indicates oversized task") {
		t.Fatalf("second prompt missing broad-scope complexity reason, got:\n%s", secondPrompt)
	}
}
