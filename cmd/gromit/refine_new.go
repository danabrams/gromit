...
var refineNewCmd = &cobra.Command{
	Use:   "refine-new [backlog-id or idea text]",
	Short: "Refine ideas into structured specs (new pipeline version)",
	Long: `Start an interactive Claude Code session to refine ideas into structured specifications.

Three input modes:
  gromit refine-new                    # Interactive picker for unrefined backlog items
  gromit refine-new <backlog-id>       # Refine a specific backlog item
  gromit refine-new "some idea text"   # Refine an ad-hoc idea (not in backlog)
...
func init() {
	rootCmd.AddCommand(refineNewCmd)
	refineNewCmd.Flags().String("agent", "", "Override the default agent for this refine session")
	refineNewCmd.Flags().Bool("choose-agent", false, "Show interactive picker to choose agent")
}

func runRefineNew(cmd *cobra.Command, args []string) error {
