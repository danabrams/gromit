...
Session worktree lifecycle and MergeBack must use one explicit owner for merge-attempt and cleanup sequencing. Conflict classification must combine git output signals and exit codes so true merge conflicts route to conflict handoff while non-conflict git failures follow cleanup + explicit error handling. Defensive abort only for merge state created in the current operation. All integration queue state changes must route through validated transitions (`ApplyTransition`) that are persisted on every error path before return; direct state assignment is forbidden. Integration queue load-mutate-write operations must run under file locking (flock/advisory lock) and persist atomically; unlocked writes are forbidden.

**Enforcement:** Code review on MergeBack path changes; integration tests validating merge state detection; contract tests for ApplyTransition coverage on error paths; file-lock assertion in queue write tests.
...
### Runtime artifacts must stay out of git; only deterministic fixtures are versioned; analytics use canonical paths
Machine-local runtime state (`.dolt/`, `.doltcfg/`, `beads_gromit/`, lock files, `.gromit/state.json`, `.gromit/stats.json`, `.gromit/interactive-state.json`) and timestamped run/report outputs must be ignored and untracked. Raw run outputs belong under ignored paths (for example `.gromit/reports/runs/`). Versioned artifacts must be deterministic fixtures under designated fixture paths; for report artifacts, use curated deterministic files (for example `.gromit/reports/curated/`). Retro/analytics readers must consume runtime artifacts from canonical repo-root locations (or persisted normalized metrics), never from session-worktree-relative paths.

**Enforcement:** Repo ignore coverage, staged-file guards in pre-commit/CI, review rejection for timestamped runtime output commits, and parity contract tests for retro ingestion in normal repo and session-worktree contexts.
