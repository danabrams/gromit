...
When a review outcome is recorded for a run (via `review guided`, `review record`, or a future API), the distiller first ensures the review packet exists, regenerating it from on-disk evidence when needed using the same recovery path defined in 0004b. It then reads the run's review artifacts plus selected run metadata, classifies the outcome's meaning using deterministic logic, and invokes an LLM to draft 3-5 structured improvement proposals. Proposals are written as `distillation-proposals.json` and `distillation-proposals.md` in the run's evidence directory. A standalone `review distill` CLI command allows re-running distillation or running it against older runs.
...
The distiller runs after `review-outcome.json` is written. In the `review guided` and `review record` code paths, after outcome persistence succeeds, the distiller is invoked. Before loading distillation inputs, the caller must ensure the review packet exists. If `product-review.json`, `process-review.json`, or `manual-checklist.json` are missing, the caller reuses 0004b's regeneration path: call `reviewpacket.InputsFromEvidence`, then `reviewpacket.Generator.Generate`, then write the packet artifacts to the evidence directory. If packet regeneration fails, the error is logged and distillation is skipped; the outcome remains persisted.
...
`review distill --run <run-id>` uses the same preamble: load run, verify `review-outcome.json` exists, ensure the review packet exists (regenerate if missing), then invoke the distiller. This keeps standalone distillation behavior aligned with the existing review commands from 0004b.
...
Prompt guidance alone is not enough; the parse-and-validate step must enforce a small amount of outcome-aware structure so acceptance criteria are testable without overconstraining the model.
...
Proposal IDs are generated from proposal content, not response position. Use a stable hash of normalized `type + title + proposed_change` and format IDs as `<run_id>-proposal-<short-hash>`. This makes IDs stable when the same proposal is regenerated and naturally produces new IDs when the proposal meaning changes.
...
The `project.json` config loaded by `cmd/gromit-next/spec.go` gains an optional `distiller_model` field (string, default `sonnet`) that specifies which LLM model the distiller uses.
...
9. If required review-packet artifacts are missing, both the automatic path and `review distill` attempt regeneration using the same `InputsFromEvidence` + `Generator.Generate` path defined in 0004b before failing.
...
11. The distiller produces 3-5 proposals per run via prompt instruction; the parse-and-validate step silently truncates to 5 if the LLM returns more. Fewer than 3 proposals is accepted as-is — the lower bound is advisory, not enforced.
...
13. Proposal IDs are generated from normalized proposal content rather than response position.
...
16. The distiller model is configurable in `project.json`, defaulting to sonnet.
17. The `reviewdistiller` package has no dependency on CLI, specloop, or stage machinery — it receives plain data and an `LLM` interface — verified by import analysis, not behavioral scenario.
...
**Then:** `distillation-proposals.json` is written with 3-5 proposals; at least one proposal is `doctrine_rule` or `planner_heuristic`; each proposal has all schema fields populated including confidence and confidence_rationale; `distillation-proposals.md` is also written
...
**Given:** a run with ID `run-104b` has `review-outcome.json`, `validation.json`, `acceptance.json`, and `review.json`, but `product-review.json` and `process-review.json` are missing because packet generation previously failed
**When:** the reviewer runs `gromit-next review distill --run run-104b`
...
**Given:** a `project.json` config with `distiller_model: haiku` and a run with ID `run-108` that has `review-outcome.json` with `outcome: "accepted"`
